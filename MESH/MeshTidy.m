...
%%
  ...
  try, [varargin,organizeTexture  ] = parseargs(varargin, 'organizeTEXture'      ,'$FORCE$',{true,organizeTexture  } ); end
  ...
  if mergeRadius == 0
    ...
    z = ~M.tri; M.tri(z) = 1;
    M.tri = reshape( map( M.tri ) ,[],size( M.tri ,2));
    M.tri(z) = 0;
    if ~keepUNUSED
      keepXYZ( loss( M.tri ) );
    end
  ...
  while ~isempty( M.tri ) && all( ~M.tri(:,end) ,1)
    M.tri(:,end) = [];
  end
  ...
  if organizeTexture && isfield( M ,'texture' ) && isfield( M ,'xyzUV' )
%     if any( M.xyzUV(:,1) < 0 ), warning('U-Texture coords are out of range (-).'); end
%     if any( M.xyzUV(:,1) > 1 ), warning('U-Texture coords are out of range (+).'); end
%     if any( M.xyzUV(:,2) < 0 ), warning('V-Texture coords are out of range (-).'); end
%     if any( M.xyzUV(:,2) > 1 ), warning('V-Texture coords are out of range (+).'); end

if 0
uv2ji = @(uv,T) [ uv(:,1) * size(T,2) + 0.5     , ( 1-uv(:,2) ) * size(T,1) + 0.5 ];
ji2uv = @(ji,T) [ ( ji(:,1) - 0.5 ) / size(T,2) , 1 - ( ji(:,2) - 0.5 )/size(T,1) ];

MS = meshSeparate( MeshTidy(M,0,true) );
for m = 1:numel( MS )
  MS{m} = MeshTidy( MS{m} ,'tex');
  MS{m}.xyzJI = uv2ji( MS{m}.xyzUV , MS{m}.texture );
end
szs = cell2mat( fun( @(M)size(M.texture,[2,1]) , MS ) );

[fn,CLEAN] = sandfile('texture_****.sizes');
printf( -fn , '%d %d\n' , szs.' );
%!pip install rectangle-packer
[~,xys] = system( sprintf( 'python -c "exec(\\"import sys, rpack\\nwith open(sys.argv[1], ''r'') as file: contenido = file.read()\\nsz = []\\nfor linea in contenido.splitlines():\\n\\tpartes = linea.split()\\n\\tsz.append( ( int( partes[0] ) , int( partes[1]) ) )\\nfor xy in rpack.pack(sz): print(f''{xy[0]} {xy[1]}'')\\")" "%s"' ,fn) );
xys = reshape( sscanf( xys ,'%d') ,2,[]).';

T = zeros( [ fliplr( max( xys + szs ,[],1) )  , 3 ] ,'uint8' );
for m = 1:numel( MS )
  T( xys(m,2) + ( 1:szs(m,2) ) , xys(m,1) + ( 1:szs(m,1) ) ,:) = MS{m}.texture;
  MS{m}.xyzJI = MS{m}.xyzJI + xys(m,:);
end
Ma = MeshAppend( MS );
Ma.texture = T;
Ma.xyzUV = ji2uv( Ma.xyzJI , Ma.texture );
Ma = MeshAddTexture( Ma );
plotMESH( Ma ,'ne','rgb','RGB');
...
%%
    ...
    uv2ji = @(uv,T) [ uv(:,1) * size(T,2) + 0.5     , ( 1-uv(:,2) ) * size(T,1) + 0.5 ];
    ji2uv = @(ji,T) [ ( ji(:,1) - 0.5 ) / size(T,2) , 1 - ( ji(:,2) - 0.5 )/size(T,1) ];
    %ji2uv( uv2ji( M.xyzUV , M.texture ) , M.texture ) - M.xyzUV
    
    ji = uv2ji( M.xyzUV , M.texture );
    v = min( ji(:,2) );       while v < 0.5, v = v + size(M.texture,1); ji(:,2) = ji(:,2) + size(M.texture,1); M.texture = cat(1,M.texture,M.texture); end
    v = max( ji(:,2) ) - 0.5; while v > size( M.texture ,1), M.texture = cat(1,M.texture,M.texture); end
    v = min( ji(:,1) );       while v < 0.5, v = v + size(M.texture,2); ji(:,1) = ji(:,1) + size(M.texture,2); M.texture = cat(2,M.texture,M.texture); end
    v = max( ji(:,1) ) - 0.5; while v > size( M.texture ,2), M.texture = cat(2,M.texture,M.texture); end


    v = floor( min( ji(:,2) ) - 0.5 ); M.texture(   1:v   ,:,:,:,:) = []; ji(:,2) = ji(:,2) - v;
    v = ceil(  max( ji(:,2) ) + 0.5 ); M.texture(   v:end ,:,:,:,:) = [];

    v = floor( min( ji(:,1) ) - 0.5 ); M.texture(:, 1:v   ,:,:,:,:) = []; ji(:,1) = ji(:,1) - v;
    v = ceil(  max( ji(:,1) ) + 0.5 ); M.texture(:, v:end ,:,:,:,:) = [];
    
    M.xyzUV = ji2uv( ji , M.texture );

%     % second pass, remove unused columns
%     ji = uv2ji( M.xyzUV , M.texture );
%     j = ji(:,1); j = round( j( M.tri ) ); j = [ min( j ,[],2) , max( j ,[],2) ];
%     J = false( 1 , size( M.texture ,2) );
%     for r = 1:size( j ,1), J( j(r,1):j(r,2) ) = true; end
%     while 1
%       j = find( ~J ,1); if isempty( j ), break; end
%       J( j ) = [];
%       M.texture( : , j , :) = [];
%       w = ji(:,1) > j-1;
%       ji( w ,1) = ji( w ,1) - 1;
...
%     M.xyzUV = ji2uv( ji , M.texture );
% 
%     % third pass, remove unused rows
%     ji = uv2ji( M.xyzUV , M.texture );
%     j = ji(:,2); j = round( j( M.tri ) ); j = [ min( j ,[],2) , max( j ,[],2) ];
%     J = false( 1 , size( M.texture ,1) );
%     for r = 1:size( j ,1), J( j(r,1):j(r,2) ) = true; end
%     while 1
%       j = find( ~J ,1,'last'); if isempty( j ), break; end
%       J( j ) = [];
%       M.texture( j , : , :) = [];
%       w = ji(:,2) > j-1;
%       ji( w ,2) = ji( w ,2) - 1;
