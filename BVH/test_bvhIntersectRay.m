...
function test_bvhIntersectRay
  ...
  for MODE = { 'first' , 'last' , 'any' }
    [ x1 , c1 , t1 ] = bvhIntersectRay( {M,B} , rays , MODE{1} );
  ...
  [ ~ , c1 , t1 , r1 ] = bvhIntersectRay( {M,B} , rays , 'all' );
  ...
  [ x1 , c1 , t1 ] = bvhIntersectRay( {M2,B2} , rays2 , 'first' );
  ...
  M = struct( 'xyz' , V , 'tri' , convhulln( V ) );
  B = BVH( M );
  [ ~ , cR , tR ] = bvhIntersectRay( {M,B} , rays , 'first' );
  for vv = { 'sphere' , 'obb' , 'kdop' , 'rss' , 'lss' }
    Bv = BVH( M , [] , vv{1} );
    [ ~ , cV , tV ] = bvhIntersectRay( {M,Bv} , rays , 'first' );
    w = cR > 0;
  ...
  [ ~ , cT , tT ] = bvhIntersectRay( {M,B} , rays , 'first' );
  wM = cM > 0;
  ...
  tic;  bvhIntersectRay( {M,B} , rays , 'first' );          tOur1 = toc;
  tic;  IntersectSurfaceRay( M , rays , 'first' );           tRef1 = toc;
  tic;  bvhIntersectRay( {M,B} , rays , 'any' );            tOur2 = toc;
